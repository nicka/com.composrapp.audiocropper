...
//Rare exceptions to indicate a bug in the _example_test_build code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_example_test_build = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_example_test_build]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.example_test_build.module"
